...
你可以在[这里]({{ url_for('probs.upload_prob') }})上传自己的题目到我们的云端，或在题目主页点击侧边栏的“上传题解”以上传你的题解。除此之外，你也可以编辑你的题目或题解。

### 题目编号的填写

上传题目前，必须填写*有意义、有区分度*的题目编号，不超过 16 个字符，建议全部使用 ASCII 字符。上传后，题目路径在 `/probs/collections/<题目编号>`。
...
在“标签”栏输入标签，然后按下回车键添加标签。除非有必要，不要创造新的标签，而是尽量使用已有的标签。在[这里]({{ url_for('probs.taglist') }})查看所有已有的标签。

### 题目描述的编写

使用 Markdown 语法编写题目描述，其中可以使用 $\KaTeX$ 公式等一些扩展语法。具体使用方法参见[此处]({{ url_for('helps.helps', howto='write-markdown') }})。
